...
const Layout = ({ children }) => {
  ...
  const filteredSidebarItems = React.useMemo(() => {
    ...
    const items = adminSidebarItems.filter(item => 
      perms.includes(`sidebar.${item.id}`) || 
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        {/* Header */}
        <div className="fixed top-0 left-0 right-0 z-40 bg-white/80 backdrop-blur-xl border-b border-gray-100/50 lg:ml-72 h-20 shadow-sm">
        ...
        <main className="flex-1 overflow-y-auto pt-24 px-6">{children}</main>
      </div>
    </div>
  );
};
