...
const iconNames = ["Users", "LineChart", "Folder", "CalendarCheck", "CheckSquare", "FileStack", "ClipboardList", "ShieldCheck"];
...
export default function BenefitsPage() {
  ...
  const [addData, setAddData] = useState({ title: "", description: "", icon: "Users" });
  ...
  const fetchBenefits = async () => {
    try {
      ...
      setErrorMessage("Failed to load benefits");
      setLoading(false);
    }
  };
  ...
  const handleEditClick = (benefit) => {
    ...
    setEditData({ title: benefit.title, description: benefit.description, icon: benefit.icon });
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrorMessage(data.message || "Failed to update benefit");
      }
    } catch (error) {
  ...
  const handleAddSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => setSuccessMessage(""), 3000);
        setIsAdding(false);
        setAddData({ title: "", description: "", icon: "Users" });
        fetchBenefits();
      } else {
        setErrorMessage(data.message || "Failed to add benefit");
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen p-6 relative">
      ...
      <div className="flex justify-center mb-6">
        <div className="hidden md:block">
          <span className="px-6 py-2 bg-blue-50/70 rounded-full text-sm font-medium text-gray-700 border border-blue-200">
            Edit Benefits (Changes Reflect on Landing Page)
          </span>
        </div>
      </div>
      ...
      <div className="text-center mb-12">
        ...
        >
          <Plus className="w-5 h-5" /> Add New Benefit
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {isAdding && (
          <div className="bg-white p-6 rounded-xl shadow-lg border-2 border-blue-500 scale-105 transform transition duration-200">
            <div className="flex justify-between items-center mb-4">
              <h4 className="font-semibold text-gray-800">New Benefit</h4>
              <button
            ...
            <div className="mb-3">
              ...
              <div className="flex items-center gap-2">
                <div className="p-2 bg-blue-50 rounded text-blue-600">
                  {iconMap[addData.icon] || <Users className="w-5 h-5" />}
                </div>
                <select
                  value={addData.icon}
            ...
            <div className="mb-3">
              ...
              <input
                type="text"
                placeholder="e.g. Robust Security"
                value={addData.title}
        ...
        {benefits.map((item, index) => (
          ...
          >
            ...
            {editingId !== item._id ? (
              <>
                <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-blue-100 text-blue-600 mb-4 transition-transform group-hover:scale-110 duration-200">
                  {iconMap[item.icon]}
                </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center mb-2">
                  <h4 className="font-semibold text-gray-800 text-sm">Edit Benefit</h4>
                  <button
