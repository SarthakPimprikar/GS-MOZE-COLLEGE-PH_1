...
const iconMap = {
  ...
  FlaskRound: <FlaskRound className="w-6 h-6" />,
  Building: <Building className="w-6 h-6" />,
  BriefcaseBusiness: <BriefcaseBusiness className="w-6 h-6" />,
...
export default function FeaturesPage() {
  ...
  const [addData, setAddData] = useState({ title: "", description: "", icon: "Globe" });
  ...
  const handleEditClick = (feature) => {
    ...
    setEditData({ title: feature.title, description: feature.description, icon: feature.icon });
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrorMessage(data.message || "Failed to update feature");
      }
    } catch (error) {
  ...
  const handleAddSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => setSuccessMessage(""), 3000);
        setIsAdding(false);
        setAddData({ title: "", description: "", icon: "Globe" });
        fetchFeatures();
      } else {
        setErrorMessage(data.message || "Failed to add feature");
      }
    } catch (error) {
      ...
      <div className="flex justify-center mb-6">
        <div className="hidden md:block">
          <span className="px-6 py-2 bg-blue-50/70 rounded-full text-sm font-medium text-gray-700 border border-blue-200">
            Edit Features (Changes Reflect on Landing Page)
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        {isAdding && (
          <div className="bg-white p-6 rounded-xl shadow-lg border-2 border-blue-500 scale-105 transform transition duration-200">
            <div className="flex justify-between items-center mb-4">
              <h4 className="font-semibold text-gray-800">New Feature</h4>
              <button
            ...
            <div className="mb-3">
              ...
              <div className="flex items-center gap-2">
                <div className="p-2 bg-blue-50 rounded text-blue-600">
                  {iconMap[addData.icon] || <Globe className="w-5 h-5" />}
                </div>
                <select
                  value={addData.icon}
        ...
        {features.map((item, index) => (
          ...
          >
            ...
            {editingId !== item._id ? (
              <>
                <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-white text-blue-600 mb-4 transition-transform group-hover:scale-110 duration-200 shadow-sm">
                  {iconMap[item.icon]}
                </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center mb-2">
                  <h4 className="font-semibold text-gray-800 text-sm">Edit Feature</h4>
                  <button
